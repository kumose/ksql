...
def test_schema_pattern(shell):
    test = (
        ShellTest(shell)
        .statement("create table goose_p (a int, b varchar, c BIT);")
        .statement("create table p_goose(d INT, f DATE);")
        .statement(".schema %p")
    )
    result = test.run()
    result.check_stdout("CREATE TABLE goose_p(a INTEGER, b VARCHAR, c BIT);")
...
def test_schema_pattern_extended(shell):
    ...
    result = test.run()
    expected = [
        "CREATE TABLE goose_p(a INTEGER, b VARCHAR, c BIT);",
        "CREATE TABLE p_goose(d INTEGER, f DATE);"
    ]
...
def test_open_non_database(shell, generated_file):
    ...
    result = test.run()
    result.check_stderr('not a valid Goose database file')
...
def test_goosebox(shell):
    test = (
        ShellTest(shell)
        .statement(".mode goosebox")

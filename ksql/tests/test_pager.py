...
def test_pager_help(shell):
    ...
    result = test.run()
    result.check_stdout('GOOSE_PAGER')
...
def test_pager_on_with_goose_pager_env(shell):
    """Test that GOOSE_PAGER takes precedence over PAGER"""
    test = (
        ShellTest(shell)
            .statement('.pager on')
            .statement('.pager')
    ...
    test.environment['PAGER'] = 'less'
    test.environment['GOOSE_PAGER'] = 'less -SR'
    result = test.run()
    result.check_stdout('less -SR')


def test_pager_goose_pager_priority(shell):
    """Test GOOSE_PAGER shows in status even when pager is off"""
    test = (
        ShellTest(shell)
            .statement('.pager on')
            .statement('.pager')
    )
    test.environment['GOOSE_PAGER'] = 'less -R'
    result = test.run()
